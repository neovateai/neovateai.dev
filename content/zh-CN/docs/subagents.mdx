...
- **neovate-code-guide** – 一个帮助回答关于 Neovate Code 本身问题的代理 - 包括其功能、配置、MCP 服务器、设置以及如何有效使用它。

## 创建自定义子代理

您可以创建自己的子代理来自动化特定工作流。子代理可以通过 Markdown 文件或插件定义。

### 文件位置

| 范围 | 位置 | 用途 |
|-------|----------|---------|
| **插件** | 通过 `agent` 钩子由插件提供 | 与 npm 包捆绑的子代理 |
...
子代理按以下顺序加载，后面的来源会覆盖前面的：

1. **内置** – 内置代理（Explore、GeneralPurpose 等）
2. **插件** – 由插件提供的代理
3. **全局** – `~/.neovate/agents/` 中的个人代理
4. **项目** – `.neovate/agents/` 中的项目特定代理（最高优先级）
