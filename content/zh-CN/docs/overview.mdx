# 概述
import PronounceNeovate from '@app/_components/pronounce-neovate'

Neovate `['niːəʊveɪt]`<PronounceNeovate />  Code 是一个开源开放的 Code Agent，同时内置最新的 AI 编程最佳实践。你可以使用它来生成代码、修复 bug、代码审查、添加测试等，也可以在交互模式或 Headless 模式下运行它，还可以通过 SDK 的方式封装自己的 Code Agent。
