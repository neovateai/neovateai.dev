...
**返回值：** `Promise<{ shutdown?: () => Promise<void> }>`

当运行 server 命令时，返回的对象包含一个 `shutdown` 函数用于优雅终止。
...
```ts
import { runNeovate, parseArgs } from '@neovate/code';

const argv = await parseArgs(['server', '--port', '3000']);
...
插件技能具有最低优先级，可被项目级技能覆盖。参见 [技能](/docs/skills) 了解 SKILL.md 格式。
    ...
    return [
      {
        ...
        description: '扫描代码中的安全漏洞。在安全审查时使用。',
        tools: ['read', 'glob', 'grep'],
        systemPrompt: '你是一位安全专家。分析代码中的漏洞...',
      },
    ];
  },
};
...
插件子代理可被全局或项目级代理覆盖。参见 [子代理](/docs/subagents) 了解更多详情。
