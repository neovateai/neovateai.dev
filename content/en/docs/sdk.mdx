...
| `options.skills` | `string[]` | Extra SKILL.md file paths for user-defined skills. Accepts absolute paths to SKILL.md files or directories (optional) |
...
You can specify additional skill paths programmatically using the `skills` option. This allows you to load user-defined skills without relying on file-based configuration or plugins.
...
Each entry can be either an absolute path to a `SKILL.md` file or a directory containing one. The system auto-detects the input type. See [Skills](./skills) for more details on skill format and usage.
