...
1. Type `/login` and select **Codex**.

2. Follow the OAuth flow to authenticate with your OpenAI account.
...
1. Type `/login` and select **Qwen**.

2. Follow the OAuth flow to authenticate with your Qwen/Alibaba Cloud account.
...
```json
{
  "provider": {
    "your-provider": {
      ...
      "models": {
        // the full list could be found in https://github.com/neovateai/neovate-code/blob/master/src/provider/modelMap.ts
        "claude-4-sonnet": {}
      },
      // optional, specify the API format. Options: "anthropic", "openai", "responses". Default: "openai"
      "apiFormat": "openai"
    }
  }
}
...
For non-builtin models, you can use an object to define the model:
