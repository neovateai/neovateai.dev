...
When running the server command, the returned object contains a `shutdown` function for graceful termination.

### Server Lifecycle

Control the server programmatically for testing or embedding scenarios:
...
// Later, gracefully shutdown the server
if (shutdown) {
  await shutdown();
}
```
