...
<Callout>
  ZenMux 每次充值额外提供 20% 的额度。
</Callout>

[ZenMux](https://zenmux.ai) 是另一个 OpenRouter 替代提供商，提供访问该平台上更新和稳定的模型。

1. 在用户控制台中打开 [ZenMux API Keys](https://zenmux.ai/settings/keys)，点击 **Create API Key** 创建新的 API 密钥并复制它。
